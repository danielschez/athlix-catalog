...
const CATEGORIA_JERSEY_ID = 23;

export default function Jersey() {
  const { productos, tallas, categorias, loading, error } = useProductos(CATEGORIA_JERSEY_ID);
  const [selected, setSelected] = useState([]);

  const subcategorias = useMemo(
    () => categorias
      .filter((c) => c.padre === CATEGORIA_JERSEY_ID)
      .map((c) => c.nombre),
    [categorias]
  );

  const tallasJersey = useMemo(
  ...
  const jerseyFilters = useMemo(() => [
    ...
    },
    {
      title: "Tallas",
      type: "buttons",
      options: tallasJersey.map((t) => t.talla),
    },
  ], [subcategorias, tallasJersey]);
  ...
  return (
    <>
      <Seo title="Jerseys Deportivos | Tienda" description="Explora nuestra colección de jerseys deportivos." />
      <Header />
      <main className="container layout">
        <Filters config={jerseyFilters} onFilterChange={setSelected} />
        <section style={{ flex: 1 }}>
          <div className="listing-header">
            <h2>Jerseys ({productosFiltrados.length})</h2>
          </div>
